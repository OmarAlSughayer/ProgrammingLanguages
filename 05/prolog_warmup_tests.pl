/* unit tests for HW 5, Prolog Warmup, CSE 341, Autumn 2015
...
test(sentence, [nondet]) :- sentence(few,students,dream).
test(sentence, [nondet]) :- sentence(all,students,cry).
test(sentence, [fail]) :- sentence(few,clams,fly).
test(sentence, [fail]) :- sentence(some,students,pass).
